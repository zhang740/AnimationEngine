(function(ae) {
    ...
    engine.Addhandler("easing", false, function(time, dom, getAttr) {
        return {
            ...
            get: ac.tween[getAttr(dom, 'type')],
            param: getAttr(dom, 'param')
        };
    }, function(time, pre, data, next) {
        if (!next) {
            return {
        ...
        if (data.value === next.value) return result;
        if (next.time < time) {
            result.value = next.value;
        } else {
            if (data.get) {
                result.value = data.value + data.get((time - data.time) / (next.time - data.time), data.param) * (next.value - data.value);
            } else {
